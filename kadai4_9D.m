...
N=zeros(Xmax,Xmax,Tmax);D=zeros(Xmax,Xmax,Tmax);Nmutant=ones(Xmax,Xmax);Nmutant(6:20,6:20)=0;%Notch•ÏˆÙ—Ìˆæ‚Ìƒpƒ^[ƒ“
...
for T=1:Tmax-1
    ...
    for X=2: Xmax-1
        for Y=2:Xmax-1
            N(Y,X,T+1)= N(Y,X,T) + dt*(( dn*( D(Y,X+1,T)+D(Y,X-1,T)+D(Y+1,X,T)+D(Y-1,X,T)) -dc*D(Y,X,T))*Nmutant(Y,X) -kn*N(Y,X,T));%Notch•ÏˆÙ—Ìˆæ‘¶Ý‰º‚É‚¨‚¢‚ÄN‚Ì‘¤•û—}§‚ðŒvŽZ
        end
    end
    for X=2: Xmax-1        
